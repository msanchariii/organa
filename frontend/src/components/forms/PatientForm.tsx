...
const PatientForm = () => {
  ...
  const onSubmit = (values: z.infer<typeof patientSchema>) => {
    ...
    const hospitalId = hospital.find((item) => item.name === values.treatingInHospital)?.id;
    const requestBody = {
      "name": values.name,
    ...
    // axios.post(routes.addPatient, requestBody)
    form.reset();
  };
                      ...
                      <CommandGroup>
                        {hospital.map((item) => (
                          <CommandItem
                            ...
                            key={item.id}
                            onSelect={() => {
                              form.setValue("treatingInHospital", item.name);
                            }}
                          >
                            {item.name}
                            <Check
